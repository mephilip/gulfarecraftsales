...
class YoFLA360Utils {
    ...
    public function is_created_with_desktop_application($src)
    {
        ...
        if( $this->is_url_path( $src ) ) return false;
    ...
    public function construct_iframe_content()
    {
        ...
        $path = '';
        if(isset($_GET['product_url'])) $path = urldecode($_GET['product_url']);
            ...
            while (false !== ($file = readdir($handle)))
            {
                if ($file != "." && $file != "..")
                {
                    ...
                    if (is_dir($file_path) && $file != YOFLA_360_TRASH_FOLDER_NAME && $file != YOFLA_360_CACHE_FOLDER_NAME)
                    {
                        if($scan_images)
                        {
                            $directories_list[] = array("name" => $file, "data" => Rotate_Tool::get_images_list($file_path));
                        }
                        else
                        {
