...
function copy_source_to_destination($path,$products_folder,$file_name,&$errors)
{
	...
	$files_excluded = getExcludedFiles($path);
	if(sizeof($files_excluded) > 0)
	{
		$errors[] = "Archive must not contain executable files!";
		return false;
	}
    ...
    elseif(sizeof($directories == 1))
    {
        ...
        if( $status )
        {
            if ( !rmdir($path) ) $errors[] = 'Failed to delete the (empty) temp. directory: '.htmlspecialchars($path);
        }
        return $status;
    }
    else{
        if( !delete_directory($path) ) { $errors[] = 'Failed to delete the temp. directory: '.htmlspecialchars($path); }
